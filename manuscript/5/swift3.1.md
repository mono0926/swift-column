タイトル: Swift 3.1 のリリースプロセスと、含まれる変更内容(前編)
...
つまり、Swift 4 では 3.0 までと比べて破壊的変更が限定的になることに加えて、上記フラグを使うことでその変更追従も先送り出来るようになっている。Xcode 8.0 以上から Swift 3系が標準となったものの、移行措置としてXcode 8.2 まで Swift 2.2 と互換性の高い 2.3 を使えるが、それと似たような対応が Swift のコンパイラーレベルでなされたとも言える。
